...
type DataStoreCall struct {
	...
	// StoreType distinguishes between "db" and "cache".
	StoreType string // "db" or "cache"

	// Operation can be a SQL query, Redis command, etc.
	Operation string

	// Key for cache operations (optional).
	Key string

	// Hit is only meaningful for cache operations. Nil if N/A.
	Hit *bool

	// ResponseTime for the operation.
	ResponseTime time.Duration
...
// TODO: Update Cache and Database properties to use DataStoreCall and unify the types
